...
void	parse_ambient(t_parse *p)
{
	...
	{
		free_parsing_fail(p, "Invalid argument for identifier: Ambient\n");
	}
	ratio = parse_float(p->tokens[1]);
	if (isnan(ratio) || ratio < 0.0 || ratio > 1.0)
	{
		free_parsing_fail(p, "Ratio range [0.0 - 1.0] or Invalid float\n");
	}
	p->sc->amb.color = parse_color(p->tokens[2]);
	if (!valid_color(&p->sc->amb.color))
	{
		free_parsing_fail(p, "Invalid color!\n");
	}
}
...
static void	parse_light_values(t_parse *p, t_light *new)
{
	...
	{
		free_lights(new);
		free_parsing_fail(p, "Wrong brightness ratio range\n");
	}
	if (arg_count(p->tokens) == 4)
...
void	parse_light(t_parse *p)
{
	...
	if (arg_count(p->tokens) != 3 && arg_count(p->tokens) != 4)
		free_parsing_fail(p, "Invalid argument for identifier: Light\n");
...
void	parse_skybox(t_parse *p)
{
	...
	{
		free_parsing_fail(p, "Invalid argument for identifier: Skybox\n");
	}
	if (ft_strlen(p->tokens[1]) < 4 || !p->tokens[1] || !*p->tokens[1])
	{
		free_parsing_fail(p, "Filename too short or empty!\n");
	}
	extension = p->tokens[1] + ft_strlen(p->tokens[1]) - 4;
	if (ft_strncmp(extension, ".xpm", 4) != 0)
	{
		free_parsing_fail(p, "Skybox must be a .xpm file\n");
	}
	p->sc->skybox_path = ft_strdup(p->tokens[1]);
	if (!p->sc->skybox_path)
	{
		free_parsing_fail(p, "Memory allocation failed for skybox path\n");
	}
}
